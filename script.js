...
for(let i=0; i<bubbleCount; i++){
  ...
  const size = Math.random() * 30 + 10; // ukuran bubble 10-40px
  bubble.style.width = `${size}px`;
  bubble.style.height = `${size}px`;
  
  bubble.style.left = `${Math.random() * 100}%`; // posisi horizontal random
  bubble.style.animationDuration = `${Math.random() * 10 + 8}s`; // durasi 8-18s
  bubble.style.animationDelay = `${Math.random() * 5}s`; // delay 0-5s
